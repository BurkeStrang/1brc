...
public class IntegrationTests
{
    ...
    public void ProcessFile_SampleData_ShouldProduceCorrectOutput()
    {
        ...
        string testDataPath = Path.Combine(GetTestDirectory(), "sample-data.txt");

        // Process the file
        string output = OneBrc.ProcessFile(testDataPath, 1);
    ...
    {
        string expected = $"{city}={expectedStats}";

        // Remove curly braces and split by comma regardless of spaces
        IEnumerable<string> entries = output.Trim('{', '}')
                            .Split(',', StringSplitOptions.RemoveEmptyEntries)
                            .Select(x => x.Trim());

        string? actual = entries.FirstOrDefault(x => x.StartsWith(city + "="));
    ...
    public void ProcessFile_SampleData_MultipleWorkers_ShouldProduceValidOutput()
    {
        ...
        string output1 = OneBrc.ProcessFile(testDataPath, 1);
        string output2 = OneBrc.ProcessFile(testDataPath, 2);
    ...
    public void ProcessFile_EdgeCases_ShouldHandleCorrectly()
    {
        ...
        try
        {
            ...
            string output = OneBrc.ProcessFile(tempFile, 1);
    ...
    public void ProcessFile_RealDataSubset_ShouldProcessWithoutErrors()
    {
        ...
        string realDataPath = Path.Combine(GetProjectRoot(), "measurements-10000000.txt");
        ...
        if (File.Exists(realDataPath))
        {
            ...
            try
            {
                IEnumerable<string> lines = File.ReadLines(realDataPath).Take(1000);
                File.WriteAllLines(tempFile, lines);

                string output = OneBrc.ProcessFile(tempFile, 2);
    ...
    public void ProcessFile_10MillionRows_ShouldProduceCorrectOutput()
    {
        ...
        string dataPath = Path.Combine(GetProjectRoot(), "measurements-10000000.txt");
        ...
        string output = OneBrc.ProcessFile(dataPath, 6);
    ...
    public void ProcessFile_10MillionRows_DifferentWorkerCounts_ShouldProduceSameOutput()
    {
        ...
        string output1 = OneBrc.ProcessFile(dataPath, 1);
        string output3 = OneBrc.ProcessFile(dataPath, 3);
        string output6 = OneBrc.ProcessFile(dataPath, 6);
    ...
    {
        DirectoryInfo? dir = new(GetTestDirectory());
